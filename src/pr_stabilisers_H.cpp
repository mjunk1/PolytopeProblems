...
if(con == true) {
	...
	if(projected_states_H(infile, con_states) != 0) {
	...
	if(use_purity == true) {
		...
		sort(con_states.begin(), con_states.end(), compare_by_purity2);

		// compute mean purity
		double mean_purity = 0;
		unsigned nstates = con_states.size();
		vector<double> purities (nstates, 0.);

		for(unsigned i=0; i<nstates; i++) {
			purities.at(i) = purity(con_states.at(i));
		...
		con_states.erase(con_states.begin()+d, con_states.end());

		cout << "   Eliminated " << nstates - con_states.size() << " points using purity criterion." << endl << endl;

		// ---- save orbits
		if(save_proj == true) {
			write_states(con_states, outfile+to_string(n)+"_con_pure_orbits.mat", outfile+to_string(n)+"_con_pure_orbits.lab");
		}
	}
...
if(n > 1 && dis == true) {
	...
	if( pr_product_states(n, prod_states, outfile+"%u_con_v.mat", outfile+"%u_con_v.lab") != 0 ) {
