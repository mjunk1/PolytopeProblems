...
for(unsigned k1=0; k1<=n/2; k1++) {
	for(unsigned k2=0; k2<=n/2-k1; k2++) {
		...
		for(unsigned i=0; i<k1; i++) {
			state = pr_product_state(pr_Bell1_T, state);
			label = pr_product_label(pr_Bell1_label_T, label);
		}
		for(unsigned i=0; i<k2; i++) {
			state = pr_product_state(pr_Bell2_T, state);
			label = pr_product_label(pr_Bell2_label_T, label);
		}
